...
import { useGetPitchByIdQuery } from '../api/pitchAPI';
...
interface FootballFieldEditFormProps {
  fieldId: string;
}
...
export default function FootballFieldEditForm({
  ...
  const methods = useForm<PitchFormData>({
    ...
    defaultValues: {
      ...
      close_time: '22:00',
      yards: [],
      images: [], // owner cần chọn lại ảnh khi tạo yêu cầu chỉnh sửa (xem lưu ý ở đầu)
    },
  });
  ...
  const onSubmit = async (data: PitchFormData) => {
    ...
    try {
      await submitPitchUpdateRequest(fieldId, data);
